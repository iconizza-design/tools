...
import {
   ...
   defaultIconProps,
} from '@iconizza/utils/lib/icon/defaults'
...
import { minifyIconSet } from '@iconizza/utils/lib/icon-set/minify'
import { convertIconSetInfo } from '@iconizza/utils/lib/icon-set/convert-info'
import type {
   ParentIconsList,
   ParentIconsTree,
} from '@iconizza/utils/lib/icon-set/tree'
import { mergeIconData } from '@iconizza/utils'
import { SVG } from '../svg'
import { defaultCommonProps, filterProps } from './props'
import type {
   CheckThemeResult,
   CommonIconProps,
   IconCategory,
...
export class IconSet {
   ...
   // Information block
   public info: IconizzaInfo | undefined
    ...
    * Resolve icon
    */
   resolve(name: string, full: false): ResolvedIconizzaIcon | null
   resolve(name: string): ResolvedIconizzaIcon | null
   resolve(name: string, full: true): Required<ResolvedIconizzaIcon> | null
   resolve(
      name: string,
      full = false,
   ): Required<ResolvedIconizzaIcon> | ResolvedIconizzaIcon | null {
      // Get parent icons tree
      const entries = this.entries
      const item = entries[name]
      const tree
      ...
      let result = {} as ResolvedIconizzaIcon
      ...
      function parse(name: string) {
         ...
         result = mergeIconData(item.props, result) as ResolvedIconizzaIcon
         if (item.type === 'icon')
    ...
    * Export icon set
    */
   export(validate = true): IconizzaJSON {
      ...
      names.forEach((name) => {
         ...
         switch (item.type) {
            case 'icon': {
               const icon: ExtendedIconizzaIcon = {
                  body: item.body,
                  ...item.props,
               }
               icons[name] = icon
            ...
            case 'variation': {
               ...
               const alias: ExtendedIconizzaAlias = {
                  parent: item.parent,
                  ...props,
               }
               aliases[name] = alias
      ...
      // Add info
      let info: IconizzaInfo | undefined
      if (this.info) {
         // Update icons count and clone object
         this.info.total = this.count()
         info = JSON.parse(JSON.stringify(this.info)) as IconizzaInfo
      }
      ...
      // Get categories
      const categories = Object.create(null) as IconizzaCategories
    ...
    * Add/update icon
    */
   setIcon(name: string, icon: ResolvedIconizzaIcon): boolean {

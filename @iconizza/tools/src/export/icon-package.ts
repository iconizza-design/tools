 ...
 */
const defaultTypesContent = `import type { IconizzaIcon } from '@iconizza/types';
declare const data: IconizzaIcon;
export default data;
`;
 ...
 *
 * Used for exporting `@iconizza-icons/{prefix}` packages
 */
export async function exportIconPackage(
	...
	const packageJSON = {
		name:
			name ||
			(esm
				? `@iconizza-icons/${iconSet.prefix}`
				: `@iconizza/icons-${iconSet.prefix}`),
		description:
			description ||
			`Iconizza icon components for ${info ? info.name : iconSet.prefix}`,
		version,
